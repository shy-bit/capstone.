...
<body>
    ...
    <section class="appointment-section" id="appointment">
        <div class="container-lg">
            <div class="appointment-container">
                ...
                <!-- 3-Day Advance Notice Alert -->
                <div class="alert alert-info" role="alert" style="grid-column: 1 / -1; margin-bottom: 20px;">
                    <i class="fas fa-info-circle me-2"></i>
                    <strong>Important:</strong> Appointments require at least 3 days advance notice. Please book your appointment accordingly to ensure availability.
                </div>

                <!-- Booking Form -->
                <form class="booking-form" action="save_booking.php" method="POST" enctype="multipart/form-data" id="bookingForm">
                    <div class="form-row">
                    ...
                    <div class="form-group">
                        <label for="appointmentDate">Appointment Date <span style="color: #dc3545;">*</span></label>
                        <input type="date" id="appointmentDate" name="appointmentDate" min="<?php echo date('Y-m-d', strtotime('+3 days')); ?>" required>
                        <small class="text-muted"> Available Tuesday to Saturday â€¢ Minimum 3 days advance notice required</small>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="time">Select Time <span style="color: #dc3545;">*</span></label>
                            ...
                            <small id="timeInfo" class="text-muted"></small>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Fetch reserved times from server
        let reservedTimes = [];
        
        function fetchReservedTimes(dateValue) {
            if (!dateValue) {
                reservedTimes = [];
                resetTimeDisplay();
                return;
            ...
            fetch(`get_reserved_dates.php?date=${dateValue}`)
        ...
        // Update time picker UI to show available/unavailable times
        function updateTimePickerUI() {
            ...
            Array.from(timeSelect.options).forEach(option => {
                if (option.value) {
                    ...
                    option.textContent = option.textContent.replace(' (Booked)', '').replace(' (Available)', '');
                    
                    // Re-add availability status
        ...
        function validateAndHighlightTime() {
            const timeSelect = document.getElementById('time');
            ...
            if (reservedTimes.includes(selectedTime)) {
                ...
                if (timeInfo) {
                    timeInfo.textContent = 'This time slot is already booked. Please select another time.';
                ...
                if (timeInfo) {
                    timeInfo.textContent = 'This time is available';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (dateInput) {
                ...
                dateInput.addEventListener('change', function() {
                    ...
                    if (selectedDate < threeDaysFromNow) {
                        alert(' Appointments require at least 3 days advance notice. Please select a date 3 or more days from today.');
                        this.value = '';
                        resetTimeDisplay();
                        return;
                    }
                    ...
                    // Check if date is Sunday (0) or Monday (1)
                    if (day === 0 || day === 1) {
                        alert('âŒ Appointments are closed on Sunday and Monday. Please select Tuesday to Saturday.');
                        this.value = '';
                        resetTimeDisplay();
                        return;
                    }
                    
                    // Fetch reserved times for this date
                    fetchReservedTimes(this.value);
                    
                    // Reset time selection
                    document.getElementById('time').value = '';
                    validateAndHighlightTime();
                });
            }
            ...
            if (bookingForm) {
                bookingForm.addEventListener('submit', function(event) {
                    ...
                    if (reservedTimes.includes(timeValue)) {
                        event.preventDefault();
                        alert('âŒ This time slot is already booked by another customer. Please select a different time.');
                        return false;
                    }
